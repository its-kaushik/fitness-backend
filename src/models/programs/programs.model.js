...
async function getLatestProgramID () {
    ...
    const latestProgram = await programsDatabase
        .findOne()
        .sort('-programID');
...
async function validateExercises(exercises) {
    ...
    for( exercise of exercises ){
        ...
        if( !exerciseExists )
            throw new Error(`No matching exercise/exercises with name ${exercise} was found`);
...
async function deleteProgramByID(programID) {
    ...
    if( !program )
        throw new Error("No Matching program found");
